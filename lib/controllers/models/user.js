...
exports.updateInfo = function(req, res, postData) {
  ...
  // var postData=req.body;
  // console.log(JSON.stringify(postData));
  console.log("{current userId: "+req.param("id")+" ; name: "+req.param("name")+" ; email: "+req.param("email")+"}");


  db.collection('users').update({"userId":parseInt(req.param("id"))},{$set:{"name":req.param("name"),"email":req.param("email")}},function(err, result) {
