          ...
          if(result[0].case == 3){
            db.collection('users').find({"userId":result[0].secondUser}).toArray(function(err, rest) {
              if (err){
                ...
                res.status(200).json({"case":3,"id":result[0].secondUser,"secondUser_name":rest[0].name,"secondUser_surname":rest[0].surname});
            ...
            if (err) {
              ...
              db.collection('users').find({"userId":parseInt(result[0].firstUser)}).toArray(function(err, rest) {
                if (err){
                  ...
                  res.status(200).json({"case":2,"id":result[0].firstUser,"firstUser_name":rest[0].name,"firstUser_surname":rest[0].surname});
                }
              });
